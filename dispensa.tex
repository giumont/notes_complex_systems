...
% =========================================================================
%                ISTRUZIONI PER I COLLABORATORI (LINEE GUIDA)
% =========================================================================
% 
% Ciao! Se stai contribuendo, intanto grazie ;) Per quanto possibile, cerchiamo di mantenere le seguenti linee guida per la coerenza stilistica. 
%
% 1. FORMATTAZIONE DEL TESTO
%    - Ipotesi di lavoro:       \underline{testo}
%    - Definizioni chiave:      \textbf{testo}
%    - Osservazioni/Note:       \emph{testo}
%
% 2. AMBIENTI MATEMATICI
%    - Formule semplici:        \[ ... \] (non numerate)
%    - Equazioni numerate:      \begin{equation} ... \end{equation}
%    - Passaggi matematici:     \begin{align} ... \end{align}
%
% 3. STRUTTURE LOGICHE
%    - Teoremi:                 \begin{theorem} ... \end{theorem}
%    - Definizioni/Note:        \textbf{Testo:} all'inizio del paragrafo.
%
% 4. ELEMENTI GRAFICI
%    - Posizionamento:          Usa [H] per bloccare le figure nel punto esatto.
%    - Organizzazione:          Caricare i file media nella cartella 'figures/'.
%    - Obbligatorio:            Inserire sempre una didascalia (\caption).
%
% 5. GESTIONE DEL CONTRIBUTO
%    - AUTORSHIP: Non fare l'umile! Aggiungi il tuo nome nel campo \author.
%    - REPO: Segui il README su GitHub per gestire Pull Requests e Commit.
%    - PULIZIA: Non caricare file ausiliari (.aux, .log, ecc.) sulla repo.
%
% 6. LEGGIBILITÀ DEL CODICE
%    - Lascia sempre una riga vuota tra un paragrafo e l'altro.
%    - Commenta i passaggi matematici complessi se necessario.
%
% =========================================================================
...
\author{Giulia Montagnani}} % Inserisci qui il tuo nome per il tuo contri
...
\begin{abstract}
Questi appunti coprono alcuni temi del corso di Introduzione alla Fisica dei Sistemi Complessi.
Il materiale è stato rielaborato ed ampliato partendo dalle lezioni frontali e non ha carattere ufficiale.
Potrebbero essere presenti refusi, errori o sezioni incomplete.
Se siete interessati a contribuire ampliando o correggendo il materiale, potete seguire le istruzioni nel README della repository
\url{https://github.com/giumont/notes_complex_systems}. Enjoy ;)
\end{abstract}
...
Assumiamo come {ipotesi di lavoro} che \underline{$P(x,t)$ sia derivabile rispetto a spazio e tempo}.  
...
Nel \underline{limite continuo} (TLC) trascuriamo i termini $o(\Delta t)$ e, confrontando con \eqref{eq:rw_master}, otteniamo
\begin{equation}
\label{eq:diff_limit}
\frac{\partial P}{\partial t}(t,x)
...
A differenza di Liouville, l’equazione di FP \emph{non mantiene tutte le correlazioni della dinamica classica}, facendo uso del teorema del limite centrale.
...
Il random walk è quindi una descrizione \emph{efficace e non fondamentale}: descrive l’evoluzione di distribuzioni di probabilità e non traiettorie microscopiche.  
A livello matematico, ciò equivale a dire che \emph{le traiettorie del random walk sono ovunque non derivabili}.
...
\begin{itemize}
    \item \underline{}{condizione iniziale determinata}
    \[
    P(0,x)=\delta(x-x_0);
    \]
    \item \underline{condizioni al contorno aperte}.
    \item \underline{sistema 1D}
\end{itemize}
...
\emph{La varianza cresce linearmente nel tempo}:
\begin{equation}
\label{eq:variance}
...
\emph{La dinamica di Fokker--Planck soddisfa un principio di aumento dell’entropia}.  
...
Qui $p(x,t)$ è una \underline{densità di probabilità}, normalizzata a ogni istante:
\[
\int dx\, p(x,t)=1.
\]
...
\end{equation}
dove le funzioni sono \underline{}{quadrato-integrabili}.  
...
Se sono in un \underline{dominio chiuso} per le funzioni (es: $[-L/2, L/2]$),
\emph{l’operatore $\mathbf{P}$ possiede sicuramente un autovalore nullo}.  
...
Queste funzioni:
\begin{itemize}
    \item hanno \emph{media nulla} (eccetto il modo costante);
    \item sono \emph{periodiche};
    \item descrivono i modi di rilassamento della distribuzione.
\end{itemize}

\paragraph{Dominio chiuso e quantizzazione}

Se il dominio è \underline{chiuso} (ad esempio condizioni periodiche), devono valere condizioni al contorno del tipo
\[
...
\emph{L’equazione rimane lineare}, quindi invariabile per combinazione lineare delle soluzioni.
...
\emph{Il coefficiente di diffusione è direttamente legato alla temperatura}.  
...
\end{equation}
che coincide con la distribuzione di \emph{Maxwell--Boltzmann}.
...
\]
Si noti che l'integrazione numerica stocastica \emph{non riproduce la singola traiettoria fisica} (che è impredicibile per natura), ma permette, campionando molte traiettorie, di ricostruire l'evoluzione della densità di probabilità $\rho(x,t)$. 
...
Nel \textbf{limite di forte attrito} (o limite \underline{overdamped}, per $m\gamma \to \infty$), il termine inerziale diventa trascurabile ($\frac{dp}{m\gamma} \to 0$). Imponendo questa condizione nella~\eqref{eq:rescaled_dp}, otteniamo un'equazione stocastica unicamente per la posizione:
\begin{equation}
...
La soluzione trovata è \emph{unica}, in quanto l'equazione differenziale del primo ordine introduce una sola costante di integrazione $A$, il cui valore è univocamente fissato dal vincolo di normalizzazione $\int \rho\, dx = 1$ (dove $A = 1/Z$, con $Z$ funzione di partizione).
